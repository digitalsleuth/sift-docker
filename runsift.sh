...
sudo docker run -d -v <host_folder_to_share>:<docker_folder_to_map>:ro --net sift_net --hostname sift --ip 172.21.0.3 --name --privileged --cap-add SYS_ADMIN --cap-add MKNOD --device /dev/fuse sift digitalsleuth/sift-docker > /dev/null
